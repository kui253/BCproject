...
int inputadmin(char* id, int x1, int y1, int charnum, int color, int size)
{
	int i = 0;
	char t;
	int flag=1;
...
int inputpassword(char* id, int x1, int y1, int charnum, int color)
{
	int i = 0;
	char t;
	clrmous(MouseX, MouseY);
		    ...
		    switch(value)
			{
				case BACK:
					...
					if(*(p-1)>31&&*(p-1)<127)
					{
						...
						bar(xx1+L_len*8-8,y1+5+Line*30-30,xx1+L_len*8,y1+5+Line*30);
						p--;
						*p='\0';
						len--;
						L_len--;
					...
					{
						setfillstyle(1,color);
						bar(xx1+L_len*(size/2+1)-size-2,y1+5+Line*30-30,xx1+L_len*(size/2+1)-1,y1+5+Line*30);
						p-=2;
						p[0]='\0';
						p[1]='\0';
						len-=2;
				...
				switch(ST)
				{
					case 1://由数字键或空格键退出输入法  输入汉字
					    if(strlen(str))//返回字符串可能为空
						{
							...
							puthz(xx1+L_len*(size/2+1),y1+5+Line*30-30,str,size,size+2,color2);
							p+=2;
					    	len+=2;
							L_len+=2;
						}
